...
function initSelects() {
  const yearIds = ['birth-year','dr-year','e-year','cpa-year','cpb-year','cal-year'];
  const monthIds = ['birth-month','dr-month','e-month','cpa-month','cpb-month','cal-month'];
  const dayIds = ['birth-day','dr-day','e-day','cpa-day','cpb-day','cal-day'];
...
function renderSaju(data) {
  ...
  elOrder.forEach(el => {
    ...
    chartHTML += `
      <div class="element-bar">
        ...
        <div class="element-bar-label" style="color:${elColors[el]}">${elNames[el]}<br><span style="color:var(--ink-faint)">${cnt}개</span></div>
      </div>`;
  });
  ...
  if (aiContainer) {
    aiContainer.innerHTML = '<div style="text-align:center;padding:20px;color:var(--ink-faint);font-size:.85rem">🤖 AI 상세 해석 생성 중...</div>';
    ...
    fetch(API + '/api/saju/ai-interpret', {
      ...
      if (aiData && aiData.ai && Object.keys(aiData.ai).length > 0) {
        ...
        aiContainer.innerHTML = '<p style="color:var(--ink-faint);font-size:.85rem;text-align:center;padding:12px">AI 해석을 불러오지 못했습니다.</p>';
      }
    }).catch((err) => {
      const msg = (err && err.name === 'TimeoutError') ? '⏱️ AI 해석 응답 시간 초과' : 'AI 해석 서비스 일시 오류';
      aiContainer.innerHTML = `<p style="color:var(--ink-faint);font-size:.85rem;text-align:center;padding:12px">${msg}</p>`;
    });
  }
}
...
function renderInterpretation(pillars, interpretation) {
  ...
  if (interp.personality) {
    ...
    const challengesHTML = (interp.challenges || []).map(c => `<li style="margin-bottom:4px;color:var(--ink-faint)">△ ${c}</li>`).join('');
    const careerHint = interp.element_info?.career || '';
    pillarHTML = `
      <div style="background:var(--bg-elevated-2);border-radius:12px;padding:16px;margin-bottom:12px">
        <div style="font-size:1rem;color:${elColor};font-family:'Noto Serif KR',serif;margin-bottom:8px">
          ${dayKey} 일주
        </div>
        <p style="color:var(--ink);line-height:1.7;margin-bottom:12px">${interp.personality}</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
          <div>
            <div style="font-size:.8rem;color:var(--ink-muted);margin-bottom:4px">강점</div>
            <ul style="list-style:none;font-size:.85rem;color:var(--ink)">${strengthsHTML}</ul>
          </div>
          <div>
            <div style="font-size:.8rem;color:var(--ink-muted);margin-bottom:4px">주의</div>
            <ul style="list-style:none;font-size:.85rem">${challengesHTML}</ul>
          </div>
        </div>
        ${careerHint ? `<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border);font-size:.83rem;color:var(--ink-muted)">${careerHint}</div>` : ''}
      </div>`;
  } else {
    pillarHTML = `<p style="color:var(--ink-muted);font-size:.9rem">일주 <strong style="color:var(--gold)">${dayKey}</strong>의 에너지를 지니고 있습니다.</p>`;
  }
  ...
  if (ai.core_nature || ai.element_balance) {
    ...
    if (ai.core_nature) {
      aiHTML += `<div style="background:rgba(184,147,74,.06);border-left:3px solid var(--gold);border-radius:0 10px 10px 0;padding:14px;margin-bottom:10px">
        <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:6px">핵심 기질 — ${dayKey} 일주만의 특성</div>
        <p style="color:var(--ink);line-height:1.75;font-size:.9rem">${ai.core_nature}</p>
      </div>`;
    }

    if (ai.element_balance) {
      aiHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:14px;margin-bottom:10px">
        <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:6px">오행 균형 분석</div>
        <p style="color:var(--ink);line-height:1.7;font-size:.88rem">${ai.element_balance}</p>
      </div>`;
    }

    if (ai.relationship_style) {
      aiHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:14px;margin-bottom:10px">
        <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:6px">인간관계 스타일</div>
        <p style="color:var(--ink);line-height:1.7;font-size:.88rem">${ai.relationship_style}</p>
      </div>`;
    }

    if (ai.career_direction) {
      aiHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:14px;margin-bottom:10px">
        <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:6px">적성과 진로 방향</div>
        <p style="color:var(--ink);line-height:1.7;font-size:.88rem">${ai.career_direction}</p>
      </div>`;
    }

    if (ai.year_2026) {
      aiHTML += `<div style="background:rgba(184,147,74,.08);border:1px solid rgba(184,147,74,.3);border-radius:10px;padding:14px;margin-bottom:10px">
        <div style="font-size:.72rem;color:var(--gold);font-weight:600;margin-bottom:6px">📅 2026년 흐름</div>
        <p style="color:var(--ink);line-height:1.7;font-size:.88rem">${ai.year_2026}</p>
      </div>`;
    }

    if (ai.growth_areas && ai.growth_areas.length) {
      aiHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:14px">
        <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:8px">성장 과제</div>
        ${ai.growth_areas.map(g => `<div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:6px">
          ...
          <p style="color:var(--ink-muted);line-height:1.6;font-size:.85rem;margin:0">${g}</p>
        </div>`).join('')}
      </div>`;
    }
  ...
  if (elInfo.temperament) {
    elemHTML = `
      <div style="background:var(--bg-elevated-2);border-radius:12px;padding:16px">
        <div style="font-size:.9rem;font-weight:600;margin-bottom:8px;color:${elColor}">
          ${primaryElHan} 기운의 특성
        </div>
        <p style="color:var(--ink);line-height:1.7;margin-bottom:10px">${elInfo.temperament}</p>
        <div style="display:grid;grid-template-columns:1fr;gap:8px;font-size:.85rem">
          ${elInfo.relationship ? `<div><div style="color:var(--ink-faint);font-size:.75rem;margin-bottom:3px">관계 패턴</div><div style="color:var(--ink)">${elInfo.relationship}</div></div>` : ''}
        </div>
      </div>`;
  }
  ...
  document.getElementById('today-advice').innerHTML = `
    <p style="color:var(--ink);line-height:1.7;margin-bottom:0">💡 ${advice || '오늘도 자신의 기운에 맞게 차분하게 움직이세요.'}</p>
    <p style="color:var(--ink-faint);font-size:.8rem;margin-top:8px;padding-top:8px;border-top:1px solid var(--border)">📅 오늘 일진: ${todayDayPillar}</p>
  `;
...
async function calcEnergy() {
  ...
  try {
    ...
    let energyHTML = energyParts.map((part, i) => {
      if (i === 0) return `<p style="margin-bottom:14px;font-size:1rem;line-height:1.8;color:var(--ink)">${part}</p>`;
      return `<p style="margin-bottom:10px;font-size:.92rem;line-height:1.7;color:var(--ink-muted)">${part}</p>`;
    }).join('');

    // 오늘 일진 정보
    if (todayPillar.pillar) {
      energyHTML += `<div style="margin-top:14px;padding:12px;background:var(--bg-elevated-2);border-radius:10px;font-size:.85rem">
        <span style="color:var(--ink-faint)">오늘 일진: </span>
        <span style="color:var(--gold);font-family:'Noto Serif KR',serif">${todayPillar.pillar}</span>
        <span style="color:var(--ink-faint);margin-left:8px">(${todayPillar.stem || ''}${todayPillar.branch || ''})</span>
      </div>`;
    }

    // 일주 성격 및 강점
    if (personality) {
      energyHTML += `<div style="margin-top:14px;padding:14px;background:rgba(184,147,74,.06);border-radius:10px;border-left:3px solid var(--gold)">
        <div style="font-size:.78rem;color:var(--ink-faint);margin-bottom:6px">나의 일주(${data.day_pillar_key || ''}) 기질</div>
        <p style="font-size:.9rem;color:var(--ink);line-height:1.7">${personality}</p>`;
      if (strengths.length) {
        energyHTML += `<div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:6px">` +
          strengths.map(s => `<span style="font-size:.78rem;background:var(--bg-elevated-2);padding:3px 10px;border-radius:50px;color:var(--ink-muted)">✦ ${s}</span>`).join('') +
          `</div>`;
      }
      energyHTML += `</div>`;
    }
    ...
    // 기질 설명
    if (myEl.temperament) {
      energyHTML += `<div style="margin-top:12px;font-size:.85rem;color:var(--ink-muted);line-height:1.6">${myEl.temperament}</div>`;
    }
    ...
    if (aiE.energy_flow || aiE.recommended_activities) {
      ...
      if (aiE.energy_flow) {
        aiEHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:14px;margin-bottom:10px">
          <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:6px">오늘의 에너지 흐름</div>
          <p style="color:var(--ink);line-height:1.75;font-size:.9rem">${aiE.energy_flow}</p>
        </div>`;
      }
      if (aiE.recommended_activities && aiE.recommended_activities.length) {
        aiEHTML += `<div style="background:rgba(122,155,110,.10);border:1px solid rgba(122,155,110,.25);border-radius:10px;padding:14px;margin-bottom:10px">
          <div style="font-size:.72rem;color:var(--wood);font-weight:600;margin-bottom:8px">✅ 오늘 추천 활동</div>
          ${aiE.recommended_activities.map((a,i) => `<div style="display:flex;gap:8px;margin-bottom:6px">
            ...
            <span style="color:var(--ink);font-size:.87rem;line-height:1.6">${a}</span>
          </div>`).join('')}
        </div>`;
      }
      if (aiE.cautions) {
        aiEHTML += `<div style="background:rgba(184,105,90,.08);border:1px solid rgba(184,105,90,.2);border-radius:10px;padding:14px;margin-bottom:10px">
          <div style="font-size:.72rem;color:var(--fire);font-weight:600;margin-bottom:6px">⚠️ 오늘 주의할 점</div>
          <p style="color:var(--ink);line-height:1.7;font-size:.87rem">${aiE.cautions}</p>
        </div>`;
      }
      if (aiE.daily_message) {
        aiEHTML += `<div style="text-align:center;padding:16px;background:rgba(184,147,74,.1);border-radius:12px;border:1px solid rgba(184,147,74,.3)">
          <div style="font-size:.72rem;color:var(--ink-faint);margin-bottom:6px">오늘의 한마디</div>
...
// ─────────────────────────────────────────────
// 정밀 사주 풀이 (deep-reading, 3대 고전 통합) — v2 신규
// ─────────────────────────────────────────────
let _drSelectedMode = 'QUICK';
const DR_MODE_LABEL = { QUICK: '빠른 풀이', STANDARD: '표준 풀이', DEEP: '심층 풀이' };
const DR_MODE_ETA_TEXT = {
  QUICK: '자평진전·적천수·궁통보감 핵심을 빠르게 풀이하고 있습니다...',
  STANDARD: '3대 고전을 종합해 표준 깊이로 풀이하고 있습니다...',
  DEEP: '세운표까지 포함한 심층 풀이를 진행하고 있습니다. 시간이 다소 걸립니다...'
};

document.addEventListener('click', function(e) {
  const modeBtn = e.target.closest('#dr-mode-segment .mode-btn');
  if (modeBtn) {
    document.querySelectorAll('#dr-mode-segment .mode-btn').forEach(b => b.classList.remove('active'));
    modeBtn.classList.add('active');
    _drSelectedMode = modeBtn.dataset.mode;
  }
});
...
async function calcDeepReading() {
  ...
  const day = +document.getElementById('dr-day').value;
  const hourVal = document.getElementById('dr-hour').value;
  const gender = document.getElementById('dr-gender').value;
  const mode = _drSelectedMode;
  ...
  document.getElementById('deep-rate-limit').style.display = 'none';
  document.getElementById('deep-loading-text').textContent = DR_MODE_ETA_TEXT[mode] || DR_MODE_ETA_TEXT.QUICK;

  // 경과 시간 타이머
  const startedAt = Date.now();
  const elapsedEl = document.getElementById('deep-elapsed');
  ...
  try {
    ...
    if (res.status === 429) {
      document.getElementById('deep-rate-limit').style.display = 'block';
      return;
    }

    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || '정밀 풀이 중 오류가 발생했습니다');

    document.getElementById('deep-result-title').textContent = `정밀 사주 풀이 결과 — ${DR_MODE_LABEL[mode] || mode}`;
    const aiText = (data.ai && (data.ai.text || data.ai.content || (typeof data.ai === 'string' ? data.ai : ''))) || '';
    const container = document.getElementById('deep-reading-content');
    if (aiText) {
      ...
      container.innerHTML = '<p style="color:var(--ink-faint)">AI 해석을 사용할 수 없어 계산된 사주 데이터만 제공됩니다.</p>';
    }

    document.getElementById('deep-disclaimer').textContent =
      data.legal || '본 내용은 명리학적 관점의 자기이해 도구이며 전문적 심리상담을 대체하지 않습니다.';

    document.getElementById('deep-result').style.display = 'block';
    showToast('✦ 정밀 사주 풀이가 완료되었습니다');
  } catch(e) {
    const msg = e.name === 'TimeoutError' ? '⏱️ 응답 시간이 초과되었습니다. 잠시 후 다시 시도해주세요.' : '❌ ' + e.message;
    showToast(msg);
  } finally {
  ...
  try {
    ...
    if (aiC.energy_dynamics || aiC.compatibility_strengths) {
      ...
      if (aiC.energy_dynamics) {
        aiCHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:12px;margin-bottom:8px">
          <div style="font-size:.7rem;color:var(--ink-faint);margin-bottom:5px">두 사람의 에너지 역학</div>
          <p style="color:var(--ink);line-height:1.7;font-size:.87rem">${aiC.energy_dynamics}</p>
        </div>`;
      }
      if (aiC.compatibility_strengths) {
        aiCHTML += `<div style="background:rgba(122,155,110,.10);border:1px solid rgba(122,155,110,.25);border-radius:10px;padding:12px;margin-bottom:8px">
          <div style="font-size:.7rem;color:var(--wood);font-weight:600;margin-bottom:5px">✅ 잘 맞는 부분</div>
          <p style="color:var(--ink);line-height:1.7;font-size:.87rem">${aiC.compatibility_strengths}</p>
        </div>`;
      }
      if (aiC.potential_conflicts) {
        aiCHTML += `<div style="background:rgba(184,105,90,.08);border:1px solid rgba(184,105,90,.2);border-radius:10px;padding:12px;margin-bottom:8px">
          <div style="font-size:.7rem;color:var(--fire);font-weight:600;margin-bottom:5px">⚠️ 갈등 가능한 부분</div>
          <p style="color:var(--ink);line-height:1.7;font-size:.87rem">${aiC.potential_conflicts}</p>
        </div>`;
      }
      if (aiC.relationship_advice && aiC.relationship_advice.length) {
        aiCHTML += `<div style="background:rgba(184,147,74,.06);border-radius:10px;padding:12px;margin-bottom:8px">
          ...
          ${aiC.relationship_advice.map((a,i) => `<div style="display:flex;gap:8px;margin-bottom:5px">
            ...
            <span style="color:var(--ink);font-size:.85rem;line-height:1.6">${a}</span>
          </div>`).join('')}
        </div>`;
      }
      if (aiC.long_term_outlook) {
        aiCHTML += `<div style="background:var(--bg-elevated-2);border-radius:10px;padding:12px">
          <div style="font-size:.7rem;color:var(--ink-faint);margin-bottom:5px">장기적 전망</div>
          <p style="color:var(--ink);line-height:1.7;font-size:.87rem">${aiC.long_term_outlook}</p>
        </div>`;
      }
      aiCHTML += `</div>`;
    }
...
async function calcCalendar() {
  ...
  try {
    ...
    data.months.forEach(m => {
      ...
      const scoreColor = m.luck_score >= 75 ? 'var(--wood)' : m.luck_score >= 65 ? 'var(--gold)' : 'var(--ink-muted)';
      gridHTML += `
        <div class="month-cal-card" data-month="${m.month}" style="cursor:pointer;background:${isCurrent?'rgba(184,147,74,.12)':'var(--bg-elevated-2)'};border:1px solid ${isCurrent?'rgba(184,147,74,.4)':'var(--border)'};border-radius:14px;padding:14px;transition:var(--transition)" onmouseenter="this.style.background='rgba(255,255,255,.10)'" onmouseleave="this.style.background='${isCurrent?'rgba(184,147,74,.12)':'var(--bg-elevated-2)'}'">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            <span style="font-weight:600;font-size:.9rem;color:${isCurrent?'var(--gold)':'var(--ink)'}">
          ...
          <div style="font-size:.78rem;color:var(--ink-muted);line-height:1.5;margin-bottom:6px">${m.advice}</div>
          <div style="font-size:.72rem;color:var(--ink-faint)">길일 ${m.lucky_day}일 · <span style="color:${elColor}">${elNames[m.month_element]||''}</span></div>
          <div style="margin-top:8px;font-size:.72rem;color:var(--ink-faint);text-align:right">📆 일별 달력 보기 →</div>
        </div>`;
    });
...
async function openMonthCalendar(targetYear, targetMonth, birthYear, birthMonth, birthDay) {
  ...
  try {
    ...
    document.getElementById('day-cal-grid').innerHTML = `<div style="grid-column:1/-1;text-align:center;color:var(--ink-muted);padding:24px">❌ ${e.message}</div>`;
  } finally {
    loading.style.display = 'none';
    body.style.display = 'block';
  }
...
function renderDayCalendar(data, targetYear, targetMonth) {
  ...
  const WEEKDAY_COLORS = ['var(--fire)', 'var(--ink)', 'var(--ink)', 'var(--ink)', 'var(--ink)', 'var(--ink)', 'var(--water)'];
  let hdrHTML = '';
  ...
  days.forEach(d => {
    ...
    const barColor = d.luck_score >= 75 ? 'var(--wood)' : d.luck_score >= 65 ? 'var(--gold)' : 'var(--ink-faint)';
    ...
    const numColor = isToday ? 'var(--bg)' : isHoliday ? 'var(--fire)' : isSat ? 'var(--water)' : 'var(--ink)';
    ...
    gridHTML += `
      <div class="day-cell" data-day="${d.day}"
        ...
        <div style="font-size:.65rem;color:${isToday?'var(--bg)':'var(--ink-faint)'};">${d.weekday}</div>
        <div style="margin-top:4px;height:3px;border-radius:2px;background:rgba(245,240,230,.1);overflow:hidden">
          <div style="height:100%;width:${barWidth}%;background:${isToday?'rgba(10,9,8,.4)':barColor};border-radius:2px"></div>
        </div>
        <div style="font-size:.6rem;color:${isToday?'rgba(10,9,8,.7)':'var(--ink-faint)'};margin-top:2px">${d.luck_score}</div>
      </div>`;
  });
...
function selectDayCell(day) {
  ...
  document.getElementById('day-cal-detail-score').innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
      <div style="font-size:.78rem;color:var(--ink-faint)">운 점수</div>
      <div style="flex:1;height:8px;border-radius:4px;background:rgba(245,240,230,.1);overflow:hidden">
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
      ...
      <span style="font-size:.78rem;color:var(--ink-muted)">${d.relation_desc || ''}</span>
      ${d.day_element ? `<span style="font-size:.72rem;color:var(--ink-faint)">[${elNames[d.day_element]||d.day_element}일]</span>` : ''}
    </div>` : ''}
    ${d.suggest_good ? `
    <div style="background:rgba(22,163,74,.08);border:1px solid rgba(122,155,110,.25);border-radius:8px;padding:8px 10px;margin-bottom:6px">
      <div style="font-size:.72rem;color:var(--wood);font-weight:600;margin-bottom:3px">✅ 추천 활동</div>
      <div style="font-size:.78rem;color:var(--ink-muted)">${d.suggest_good}</div>
    </div>` : ''}
    ${d.suggest_avoid ? `
    <div style="background:rgba(184,105,90,.09);border:1px solid rgba(184,105,90,.2);border-radius:8px;padding:8px 10px">
      <div style="font-size:.72rem;color:var(--fire);font-weight:600;margin-bottom:3px">⚠️ 주의 활동</div>
      <div style="font-size:.78rem;color:var(--ink-muted)">${d.suggest_avoid}</div>
    </div>` : ''}
  `;
...
async function loadSajuHistory() {
  ...
  try {
    ...
    let html = '<div style="font-size:.78rem;font-weight:700;color:var(--ink-faint);letter-spacing:.06em;margin-bottom:10px">📜 최근 본 사주</div>';
    data.history.forEach(h => {
      ...
      html += `
        <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg-elevated-2);border-radius:10px;margin-bottom:6px;cursor:pointer;transition:background .2s"
          onclick="loadHistoryItem(${h.birth_year},${h.birth_month},${h.birth_day},${h.birth_hour||'null'},'${h.gender||'male'}')"
          onmouseover="this.style.background='rgba(184,147,74,.1)'"
          ...
          <div style="flex:1">
            <div style="font-size:.85rem;color:var(--ink)">${h.birth_year}년 ${h.birth_month}월 ${h.birth_day}일${h.birth_hour!=null?' '+h.birth_hour+'시':''}</div>
            <div style="font-size:.72rem;color:var(--ink-faint)">${elName} ${dateStr}</div>
          </div>
          <div style="font-size:.75rem;color:var(--ink-faint)">▶</div>
        </div>`;
    });
...
function loadHistoryItem(year, month, day, hour, gender) {
  ...
  // 나의 사주 탭으로 이동
  const sajuTab = document.querySelector('.tab[data-tab="saju"]');
...
function renderAiInterpretation(ai, containerOrIgnored) {
  ...
  if (ai.core_nature) {
    html += `<div class="ai-section" style="border-left:3px solid var(--gold);border-radius:0 10px 10px 0;background:rgba(184,147,74,.06)">
      <h4>핵심 기질</h4>
      <p>${ai.core_nature}</p>
    </div>`;
  }
